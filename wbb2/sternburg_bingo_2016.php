...
//
// Sternburg Bingo 2016 v1.0 by kill0rz
//
...
$filename = "sternburg_bingo_2016.php";
...
if ($loggedin) {
	...
	$result = $db->unbuffered_query("SELECT * FROM bb1_sternburg_bingo_2016_fields GROUP BY field_nr ORDER BY field_nr ASC");
	while ($row = mysqli_fetch_object($result)) {
		$field_treffer_xy = array(array());
		$field_treffer_yx = array(array());

		// suche für jede Fieldnummer die zaheln und ordne sie an
		$result2 = $db->unbuffered_query("SELECT * FROM bb1_sternburg_bingo_2016_fields WHERE field_nr='" . $row->field_nr . "' ORDER BY pos_y ASC, pos_x ASC");
	...
	foreach ($user_kronkorken as $key => $number) {
		...
		if ($canedit) {
			$has_kronkorken .= '<div class="sammlung"><a href="./sternburg_bingo_2016.php?action=del&amp;number=' . $number . '"><img src="img/sternburg_bingo_2016/sternburg_deckel.png" alt="" /> <h1>' . str_pad($number, 2, '0', STR_PAD_LEFT) . '</h1></a></div>';
		} else {
			$has_kronkorken .= '<div class="sammlung"><img src="img/sternburg_bingo_2016/sternburg_deckel.png" alt="" /> <h1>' . str_pad($number, 2, '0', STR_PAD_LEFT) . '</h1></div>';
		}
	}
...
eval("\$tpl->output(\"" . $tpl->get("sternburg_bingo_2016") . "\");");
